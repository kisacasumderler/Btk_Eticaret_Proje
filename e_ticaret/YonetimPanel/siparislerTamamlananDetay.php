...
<div class="BtnDiv">
    <a href="index.php?SKD=0&SKI=108" class="addBtn">Tamamlanan Siparişlere geri dön</a>
</div>
<ul>
    <?php 
        $siparislerSorgu = $db->prepare("SELECT * FROM siparisler WHERE OnayDurumu=? AND siparisNumarasi=? AND kargoDurumu=?");
        $siparislerSorgu->execute([1,$GelenSepetNo,1]);
    ...
    <div class="flexRowStart" style="padding: 1rem; margin: 2rem 0;">
            ...
            <?php
             foreach($siparisKayitlar as $Values){
                ...
                if (DonusumleriGeriDondur($Values["urunTuru"])=="Kadın Ayakkabısı") {
                 ...
                 $kargoGonderiKodu = DonusumleriGeriDondur($Values["kargoGonderiKodu"]);
        ...
        <div class="width50 flexColumnCenter gap1 UrunDetaySagTaraf">
            <div style="background: #04263d; color: #fff;">
                <span class="ListBaslik" style="font-size : 1.2rem;">Kargo Takip No : </span>
                <span><?php echo $kargoGonderiKodu;?></span>
            </div>
            <div>
